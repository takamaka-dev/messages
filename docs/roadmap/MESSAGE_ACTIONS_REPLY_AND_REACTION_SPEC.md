...
**Related strict-drop case — embedded inner signature failure.** The unknown-action strict drop is one member of a strict-drop family. The other current member is **`InnerSignatureFailureException`** triggered when `action="share_history"` carries an `original_message` whose embedded Ed25519 signature does not verify against the embedded `from` PK (see §12.11.4 step 2). Both are HARD violations: validator throws, message dropped entirely, no rendering, no decoration-attached body. The reasoning is symmetric — a message whose cryptographic identity cannot be trusted (signature failure) or whose action's structural identity cannot be interpreted (unknown action) is unsafe to render. Data integrity takes precedence over informational decoration. See §13 decisions 8 and 11.
...
| `share_history` whose embedded inner signature does not verify | **Message dropped (HARD violation).** Validator throws `InnerSignatureFailureException` extending `HardProtocolViolationException`. Decoration code `INNER_SIGNATURE_FAILURE`. Outer wrapper does NOT render — signature failure on the embedded envelope indicates corruption or tampering; either way, the inner cargo cannot be trusted and the outer wrapper has no useful content without it. See §12.7-equivalent strict-drop family. |
...
The visible signs of this design are exactly what you would predict. `targets[0]` carries a public key rather than a message signature, because nothing about the original message would resolve to a signature in the destination. Cardinality is zero-or-one: forwards may either claim an origin or stay anonymous. **Chain provenance is preserved recursively through `fw_content`, up to a hard depth cap of 10** — each forwarder carries the previous forwarder's decrypted content, so a downstream recipient can inspect the full attribution trail rather than getting only the most recent claim. This is the accountability-by-trail framing: legitimate forwards have no friction, but a forward that's been passed around many times shows that fact to its recipients, and recipients can apply their own social judgment. Clients are required to visually distinguish forwarded content from natively-signed messages, because rendering them identically would be the UI lie that this design exists to prevent. **The decrypted structural context of each layer is preserved** — if a layer was a reply or a reaction or an edit, that fact travels with that layer's `fw_content`. The referenced targets will typically be broken-reference in the destination (they point at signatures from a conversation the receiver isn't in), but the structural intent is preserved. The receiver sees what each layer originally tried to be without cryptographic backing — honest middle ground: don't strip the intent, don't claim verification. The design is **cryptographically minimal** (no signatures or provenance travel) and **structurally preserving** (the chain of decrypted content and claimed attribution IS preserved). The trust model is screenshot-equivalent — content signed by the most recent forwarder, attribution claimed without cryptographic backing — but the content travels with structural fidelity, so the recipient can see what was being passed around and how many hands it passed through.
...
3. **Cost disclosure.** Forwarding a 1 GB video means uploading 1 GB again. There is no protocol-level deduplication path that preserves zero-knowledge — fresh per-upload IV/salt means even identical plaintexts produce different `encrypted_file_hash` values. Client UIs SHOULD warn the user before forwarding large attachments. Because re-encryption is recursive (below), a deeply-nested forward re-uploads every layer's attachments at every hop.

4. **Placement — per-layer, never flattened (normative).** "The forward message's `attached_media`" means the `attached_media` of the **layer that carries the attachment**, relinked **in place** — for a forwarder-authored attachment, the forward's own `attached_media`; for an attachment belonging to a forwarded (nested) message, that message's `attached_media` inside the corresponding `fw_content` layer. Re-encrypted attachments **MUST NOT be hoisted/flattened into a single top-level list**: doing so severs the per-layer comment↔attachment binding that nesting exists to preserve. (Example: an inner *"don't use this picture" + pic₁* wrapped by an outer *"use this picture" + pic₂* must stay `{text:"use this picture", media:[pic₂], fw_content:{text:"don't use this picture", media:[pic₁]}}`; flattened to `media:[pic₁,pic₂]` the comments no longer resolve to a specific picture.) Re-encryption is therefore **recursive**: every `fw_content` layer's regular attachments are re-encrypted under the destination key and kept in that layer, bounded by the depth-10 cap (§12.10.3). Inline (`isTheObject == true`) attachments need no re-upload — their plaintext rides in the re-encrypted layer body — but their `unencrypted_content_hash` is still recomputed per step 1.

5. **Receiver requirement (normative).** Because re-encrypted attachments live in their layer (including nested `fw_content` layers), receivers MUST collect media placeholders **recursively across `fw_content`** — not only the top-level `attached_media` — when building their downloadable-attachment index; otherwise a forwarded attachment is decryptable (it is bound to the destination key) yet never surfaceable for download.
...
- **Chain provenance via `fw_content` (RECURSIVE — normative).** Each forward's `fw_content` field carries the entire decrypted inner content of the message being forwarded — including, recursively, that message's own `fw_content` if it was itself a forward. A forward of a forward carries `fw_content.fw_content...`, up to a hard depth cap of 10. At depth > 10, receivers MUST truncate the deepest `fw_content` branch with decoration `forward depth exceeded — chain truncated at level 10`. Forwarders SHOULD flatten or truncate before sending if their forward would exceed the cap. Each layer's claimed-origin PK in `targets[0]` (cardinality 0 or 1 per layer) is preserved alongside its decrypted content, so a recipient at depth N can inspect the chain of attributions and decrypted bodies all the way back to depth 0. This recursive preservation is the **accountability-by-trail design**: the wire carries the metadata of who has been passing this around, and the recipient can apply their own social judgment.
...
- **The protocol does not incentivize forwarding over native composition.** A forward is **cryptographically a screenshot** — the structural content of the original (and the chain of preceding forwarders' content, recursively via `fw_content`) is preserved to enable accountability-by-trail, but no cryptographic provenance accompanies any of it. Composing the same content natively in the destination conversation — where the author signs their own envelope and the recipients verify it — is always the cryptographically stronger option, and the spec prefers it.
...
2. **Verify the embedded inner signature** against `original_message.from`. Use the same Ed25519 verification machinery as the outer envelope. **Mismatch → HARD VIOLATION: validator throws `InnerSignatureFailureException` and the entire share_history message is dropped — outer wrapper does NOT render.** Decoration code `INNER_SIGNATURE_FAILURE`. Rationale: a failed signature on the embedded envelope is indistinguishable from tampering and from local-database corruption on the relayer's side. In either case, the inner cargo has lost cryptographic provenance and the outer wrapper has no useful content without it. Forwarding corrupted data to downstream participants would only propagate the corruption; rejecting the message forces the relayer (or their corrupted database) to surface the failure and be fixed. This is consistent with the §12.7 strict-drop family — data integrity is prioritized over render-anyway-with-decoration. See §13 decision 11.
...
| 10 | Validator hard/soft split | **Hard violations throw `HardProtocolViolationException` subclass; message dropped entirely.** Soft violations return `ValidationResult` with `List<Decoration>` and severity (INFO/WARN/ERROR); body still renders. Cross-platform: Dart mirrors the exception hierarchy and the record shape. Validator's step 0 (version gate) and step 1 (action recognition) throw on hard violations before any rendering decision is made. |
| 11 | `share_history` inner-signature failure: hard or soft? | **HARD** (§12.7 strict-drop family, §12.11.4 step 2). A failed signature on the embedded envelope cannot be distinguished from tampering vs corrupted-local-cache; both are unsafe to propagate. Validator throws `InnerSignatureFailureException`; entire share_history message is dropped (including the outer wrapper). Decoration code `INNER_SIGNATURE_FAILURE`. **Data integrity takes precedence over informational decoration** — corrupted data must not be forwarded downstream. Scope kept narrow: only cryptographic signature failures are promoted to hard. Other tamper-signal soft cases (inline hash mismatch, share_history cross-conversation) remain soft with severity ERROR. |
| 12 | Polls, read receipts, typing, vote, acknowledge | Deferred (§12.6). |
| 13 | `conversation_role` column | Dead data (§6.6). Cleanup tracked separately, not blocking this branch. |
